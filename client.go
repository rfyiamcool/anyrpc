...
func (c *Client) call(fn reflect.Value, name string, in []reflect.Value) []reflect.Value {
	...
	if err != nil {
		return c.returnCallWithError(fn, err)
	}
	...
	if e != nil {
		return c.returnCallWithError(fn, e)
	}

	if n != name {
		return c.returnCallWithError(fn, fmt.Errorf("rpc name %s != %s", n, name))
	}
	...
	if last != nil {
		if err, ok := last.(error); ok {
			...
			return c.returnCallWithError(fn, fmt.Errorf("rpc final return type %T must be error", last))
		}
	}
...
func (c *Client) returnCallWithError(fn reflect.Value, err error) []reflect.Value {
	nOut := fn.Type().NumOut()
